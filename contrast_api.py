...
def load_config(file_path="config.json", file=None) -> list[dict]:
    try:
        if file:
...
class ContrastTeamServer:
    ...
    def list_org_apps(
        ...
        call = org_id + "/applications/filter?expand=license,metadata&sort=appName"
        body = {"quickFilter": quick_filter}
        if include_merged:
    ...
    def test_org_access(self, orgId=None):
        """Check we can access the specified organization."""
        if not orgId:
            orgId = self._org_uuid
...
def contrast_instance_from_json(json: dict) -> ContrastTeamServer:
    ...
    return ContrastTeamServer(
        url, json["apiKey"], json["authorizationHeader"], json["orgId"]
    )
